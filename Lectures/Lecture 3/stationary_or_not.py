...
def stationary_bayes_filter(data_set):
    ...
    for i in range(1, len(data_set)):
        ...
        # Prediction
        p_stationary_predicted = conditional_prob_stationary_given_moving() * p_stationary_tm1 + conditional_prob_stationary_given_stationary() * p_stationary_tm1 
        p_moving_predicted = conditional_prob_moving_given_moving() * p_moving_tm1 + conditional_prob_moving_given_stationary() * p_moving_tm1

        # Corrrection
        eta = 1/(conditional_prob_z_vel_given_stationary(z_vel) * p_stationary_predicted + conditional_prob_z_vel_given_moving(z_vel) * p_moving_predicted)

        p_stationary = eta * conditional_prob_z_vel_given_stationary(z_vel) * p_stationary_predicted
        p_moving = eta * conditional_prob_z_vel_given_moving(z_vel) * p_moving_predicted
