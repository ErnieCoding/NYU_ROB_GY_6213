...
void setup() {
  ...
  if (IS_OK(lidar.getDeviceInfo(info, 1000))) {
    Serial.println("LIDAR device info OK");
    if (IS_OK(lidar.startScan())) {
      Serial.println("startScan OK");
    } else {
      Serial.println("startScan FAILED");
    }
  } else {
...
void loop() {
  if (IS_OK(lidar.waitPoint())) {
    ...
    Serial.println("waitPoint FAILED");
    ...
    rplidar_response_device_info_t info;
    if (IS_OK(lidar.getDeviceInfo(info, 1000))) {
      Serial.println("device rediscovered");
      analogWrite(RPLIDAR_MOTOR, 255);
      delay(1000);

      if (IS_OK(lidar.startScan())) {
        Serial.println("restart scan OK");
      } else {
        Serial.println("restart scan FAILED");
      }
    } else {
      Serial.println("device info FAILED after waitPoint failure");
    }
  }
}